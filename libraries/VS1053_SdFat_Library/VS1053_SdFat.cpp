...
static VS1053_SdFat_FilePlayer *myself;
...
boolean VS1053_SdFat_FilePlayer::useInterrupt(uint8_t type) {
  myself = this; // oy vey
...
VS1053_SdFat_FilePlayer::VS1053_SdFat_FilePlayer(int8_t rst, int8_t cs,
                                                       int8_t dcs, int8_t dreq,
                                                       int8_t cardcs)
    : VS1053_SdFat(rst, cs, dcs, dreq) {

  playingMusic = false;
  _cardCS = cardcs;
}

VS1053_SdFat_FilePlayer::VS1053_SdFat_FilePlayer(int8_t cs, int8_t dcs,
                                                       int8_t dreq,
                                                       int8_t cardcs)
    : VS1053_SdFat(-1, cs, dcs, dreq) {

  playingMusic = false;
  _cardCS = cardcs;
}

VS1053_SdFat_FilePlayer::VS1053_SdFat_FilePlayer(int8_t mosi, int8_t miso,
                                                       int8_t clk, int8_t rst,
                                                       int8_t cs, int8_t dcs,
                                                       int8_t dreq,
                                                       int8_t cardcs)
    : VS1053_SdFat(mosi, miso, clk, rst, cs, dcs, dreq) {

  playingMusic = false;
  _cardCS = cardcs;
}

boolean VS1053_SdFat_FilePlayer::begin(void) {
  // Set the card to be disabled while we get the VS1053 up
  pinMode(_cardCS, OUTPUT);
  digitalWrite(_cardCS, HIGH);

  uint8_t v = VS1053_SdFat::begin();
...
boolean VS1053_SdFat_FilePlayer::playFullFile(const char *trackname) {
...
void VS1053_SdFat_FilePlayer::stopPlaying(void) {
  // cancel all playback
...
void VS1053_SdFat_FilePlayer::pausePlaying(boolean pause) {
...
boolean VS1053_SdFat_FilePlayer::paused(void) {
  return (!playingMusic && currentTrack);
}

boolean VS1053_SdFat_FilePlayer::stopped(void) {
  return (!playingMusic && !currentTrack);
}

// Just checks to see if the name ends in ".mp3"
boolean VS1053_SdFat_FilePlayer::isMP3File(const char *fileName) {
  return (strlen(fileName) > 4) &&
         !strcasecmp(fileName + strlen(fileName) - 4, ".mp3");
}

unsigned long VS1053_SdFat_FilePlayer::mp3_ID3Jumper(File mp3) {
...
boolean VS1053_SdFat_FilePlayer::startPlayingFile(const char *trackname) {
  // reset playback
  sciWrite(VS1053_REG_MODE, VS1053_MODE_SM_LINE1 | VS1053_MODE_SM_SDINEW |
...
void VS1053_SdFat_FilePlayer::feedBuffer(void) {
  noInterrupts();
...
void VS1053_SdFat_FilePlayer::feedBuffer_noLock(void) {
...
VS1053_SdFat::VS1053_SdFat(int8_t mosi, int8_t miso, int8_t clk,
...
VS1053_SdFat::VS1053_SdFat(int8_t rst, int8_t cs, int8_t dcs,
                                 int8_t dreq) {
  _mosi = 0;
  _miso = 0;
  _clk = 0;
...
void VS1053_SdFat::applyPatch(const uint16_t *patch, uint16_t patchsize) {
  uint16_t i = 0;
...
uint16_t VS1053_SdFat::loadPlugin(char *plugname) {
...
boolean VS1053_SdFat::readyForData(void) { return digitalRead(_dreq); }

void VS1053_SdFat::playData(uint8_t *buffer, uint8_t buffsiz) {
...
void VS1053_SdFat::setVolume(uint8_t left, uint8_t right) {
...
uint16_t VS1053_SdFat::decodeTime() {
  noInterrupts(); // cli();
...
uint8_t VS1053_SdFat::begin(void) {
  if (_reset >= 0) {
...
void VS1053_SdFat::dumpRegs(void) {
...
uint16_t VS1053_SdFat::recordedWordsWaiting(void) {
  return sciRead(VS1053_REG_HDAT1);
}

uint16_t VS1053_SdFat::recordedReadWord(void) {
  return sciRead(VS1053_REG_HDAT0);
}

boolean VS1053_SdFat::prepareRecordOgg(char *plugname) {
...
void VS1053_SdFat::stopRecordOgg(void) { sciWrite(VS1053_SCI_AICTRL3, 1); }

void VS1053_SdFat::startRecordOgg(boolean mic) {
...
void VS1053_SdFat::GPIO_pinMode(uint8_t i, uint8_t dir) {
  if (i > 7)
    return;
...
void VS1053_SdFat::GPIO_digitalWrite(uint8_t val) {
  sciWrite(VS1053_REG_WRAMADDR, VS1053_GPIO_ODATA);
  sciWrite(VS1053_REG_WRAM, val);
}

void VS1053_SdFat::GPIO_digitalWrite(uint8_t i, uint8_t val) {
  if (i > 7)
    return;
...
uint16_t VS1053_SdFat::GPIO_digitalRead(void) {
  sciWrite(VS1053_REG_WRAMADDR, VS1053_GPIO_IDATA);
  return sciRead(VS1053_REG_WRAM) & 0xFF;
}

boolean VS1053_SdFat::GPIO_digitalRead(uint8_t i) {
  if (i > 7)
    return 0;
...
uint16_t VS1053_SdFat::sciRead(uint8_t addr) {
  uint16_t data;
...
void VS1053_SdFat::sciWrite(uint8_t addr, uint16_t data) {
#ifdef SPI_HAS_TRANSACTION
  if (useHardwareSPI)
    SPI.beginTransaction(VS1053_CONTROL_SPI_SETTING);
#endif
...
uint8_t VS1053_SdFat::spiread(void) {
  int8_t i, x;
  x = 0;
...
void VS1053_SdFat::spiwrite(uint8_t c) {

  uint8_t x __attribute__((aligned(32))) = c;
  spiwrite(&x, 1);
}

void VS1053_SdFat::spiwrite(uint8_t *c, uint16_t num) {
  // MSB first, clock low when inactive (CPOL 0), data valid on leading edge
  // (CPHA 0) Make sure clock starts low
...
void VS1053_SdFat::sineTest(uint8_t n, uint16_t ms) {
  reset();
